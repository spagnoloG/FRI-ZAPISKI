...
\begin{multicols}{2}
	...
	Naj bo \( \tau: V \rightarrow U \) linearna preslikava, \(\dim V = \dim U = n\) in
	naj bo \(A\) neka matrika, ki pripada \(\tau\). Naslednje trditve so ekvivalentne:
	...
	\subsubsection{Izrek}
	Dvakrat zvezno odvedljiva funkcija \( f: D \subseteq \mathbb{R}^n \rightarrow \mathbb{R} \) je:
	\begin{itemize}
		\item \textbf{konveksna} ($\cup$) natanko tedaj, ko je \( \nabla^2 f(x) \) pozitivno semidefinitna matrika na \( D \),
		\item \textbf{konkavna} ($\cap$) natanko tedaj, ko je \( \nabla^2 f(x) \) negativno semidefinitna matrika na \( D \).
	\end{itemize}
	...
	V primeru, ko velja \(d^\star = p^\star\), morajo \(\vec{x}^\star\), \(\vec{\lambda}^\star\) in \(\vec{\mu}^\star\) zadostiti Karush--Kuhn--Tuckerjevim pogojem:
	\[
		\begin{aligned}
